...
tcplHit2 <- function(mc4, coff) {
  ...
  test <- nested_mc4 %>%
    dplyr::rowwise() %>%
    mutate(df = list(tcplhit2_core(params = params, conc = unlist(conc), resp = unlist(resp), bmed = bmed, cutoff = coff, onesd = onesd, bmd_low_bnd = min(conc) * 0.1 ,bmd_up_bnd = max(conc) * 10))) %>%
    select(-conc, -resp)
  # test <- NULL
