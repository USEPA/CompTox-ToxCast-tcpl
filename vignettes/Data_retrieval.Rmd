...
Users must be connected to the <font face="CMTT10">tcpl</font> database or a replicate of the database to utilize many of these functions and execute the examples in this vignette.  Please see the introductory vignette in this package for more details.
...
Throughout this vignette we will use abbreviated designations for data retrieved from the database or to refer to processing steps within <font face="CMTT10">tcpl</font>. For data from single concentration assays we use 'SC' and 'MC' is used the data assay has multiple concentrations. A particular data or processing level is indicated by appending the level id/number to the end of the 'SC' or 'MC' designation. For example, if we are discussing single concentration data from level 2 processing, then we will use the abbreviation 'SC2'.
...
The <font face="CMTT10">tcplLoadData</font> function can be used to load the data from the MySQL database into the R session. Furthermore, the <font face="CMTT10">tcplPrepOtpt</font> function can be used in combination with <font face="CMTT10">tcplLoadData</font> to prepare retrieved data into a readable output, and query the database for additional chemical and assay annotation information to map to the retrieved data.

```{r tcplPrepOtpt, eval=FALSE}
# Load single concentration data from level 1, put into a readable format,
...
The <font face="CMTT10">tcplMthdLoad</font> function returns the method assignments for specified id(s). Later sections provide more detailed examples for utilizing the method loading functions for individuals ids.

# Retrieving Level 0 Data

Prior to the pipeline processing provided in this package, all the data must go through pre-processing, i.e. raw data to database level 0 data. Pre-processing the data should transform data from heterogeneous assays into a uniform format.  This is executed using dataset specific R scripts.  After pre-processing is complete and the formatted data matches the level 0 format it can be loaded into the <font face="CMTT10">tcpl</font> database. The standard level 0 format is identical for both testing paradigms, single concentration (sc) or multiple concentrations (mc). Users can inspect the level 0 data and calculate assay quality metrics prior to running the processing pipeline.
...
# Alternatively, load data in and format to a readable with tcplPrepOtpt.
...
Since we are not able to connect to the database directly, we have provided a sample dataset in the package to illustrate what the results should look like.
...
The goal of single concentration processing is to identify potentially active compounds from a large screen at a single concentration. After processing, users can inspect single concentration activity hit calls and the applied methods.
...
The goal of multiple-concentration processing is to estimate the activity, potency, efficacy, and other endpoints for sample-assay pairs given a series of multiple concentrations. After processing, users can inspect the activity hit calls, model parameters, concentration-response plots, and the applied methods for the multiple concentration data. 

## Load MC5 Data

```{r mc5_data, eval = FALSE}
# Load Level 5 multiple concentration data summary values for a set of aeids.
...
# Create a function to load methods for multiple concentration data processing
# for select aeids.
...
# Plot Level 5 multiple concentration data for select aeids.
...
In this example, we illustrate the necessary steps for extracting information about the compound *Bisphenol A* found within the <font face="CMTT10">tcpl</font> database. The user will define the chemical of interest, isolate all associated sample ids ($\mathit{spids}$), and then load all data for the given chemical. 
