...
In <font face="CMTT10">tcpl v3.0</font> onwards, a single plotting function with many customizable options, <font face="CMTT10">tcplPlot</font>, allows for interactive, yet consistent visualization of concentration-response curves. As a new standalone plotting utility built with the R library plotly to display the additional curve-fitting models, <font face="CMTT10">tcplPlot</font> implements the R library plumber to provide representational state transfer-application programming interface (REST API) functionality, which also works with web applications. Plotly concentration-response plots are akin to the bioactivity plots surfaced via the CompTox Chemicals Dashboard. The <font face="CMTT10">tcplPlot</font> function requires a level (`lvl`), field (`fld`), and value (`val`) input combination to load the necessary data and display the associated plots and tables as `output`. The plotting utility supports a variety of publication-quality file type options, including raster graphics (PNG, JPG, and TIFF) to retain color quality when printing to photograph and vector graphics (SVG and PDF) to retain image resolution when scaled to large formats.

Further customization of outputs is possible via parameters, `multi` and `verbose`. The `multi` parameter allows for single or multiple plots per page. The PDF `output` option must be selected to generate multiple plots per page. The `verbose` parameter results in a plot that includes a table containing potency and model performance metrics and is available for all output types aside from "console".

## Plotting Examples

This section demonstrates <font face="CMTT10">tcplPlot</font> functionality. For all but the first two examples, an image of the generated output follows the corresponding code block.

Due to the dynamic nature of "m#" fields, the following example does not include a corresponding plot. Here, the $m4id$ value (482273) corresponds with the <font face="CMTT10"> mc_vignette </font> R data object. To run test this code, please select a valid $m4id$ value.
```{r mc_plot_ma4id, eval = FALSE}
# Create Level 4 plot for a single m4id.
tcplPlot(lvl = 4,  # data level
         fld = "m4id", # field to query on 
         val = 482273, # value for each field, values should match their corresponding 'fld'
...
The following two examples produce plots of Level 5 MC data for the selected $aeids$. The a new pdf is generated for each $aeid$ and contains multiple plots per page. Example output from the second <font face="CMTT10">tcplPlot</font> call follows this code block.
...
tcplPlot(lvl = 5, # data level
         ...
         fileprefix = "output_") # prefix of the filename

# Loading required mc_vignette data for example below
data(mc_vignette, package = 'tcpl')
mc5 <- mc_vignette[["mc5"]]

# Plot Level 5 MC data from the mc_vignette R data object for a single aeid 80 and spids
# "TP0001652B01", 01504209", "TP0001652D01", "TP0001652A01", and "1210314466" 
tcplPlot(lvl = 5, # data level
         fld = c("aeid", "spid"), # field to query on
         val = list(mc5$aeid, mc5$spid), # value for each field, 
                                         # values should match their corresponding 'fld'
         by = "aeid", # parameter to divide files
         multi = TRUE, # multiple plots per page - output 6 per page if TRUE
...
```
<center>![<font style="font-size:15px"><i>Plots with parameters: output = "pdf", multi = TRUE, and verbose = TRUE</i></font>](img/output_pdf.png)</center>


This example illustrates a level 5 verbose plot for $aeid$ 80 and $spid$ 01504209 of output type "jpg".
```{r mc_plot_jpg, eval = FALSE}
# Plot a verbose plot of Level 5 MC data for single aeid and spid and output as jpg.
tcplPlot(lvl = 5, # data level
         fld = c('aeid','spid'), # field to query on
         val = list(80,'01504209'), # value for each field
         # values should match their corresponding 'fld'
         multi = FALSE, # single plot per page, TRUE not valid with output = jpg
         verbose = TRUE, # output all details
         output = "jpg", # output as jpg
         fileprefix = "output_jpg")
```
<center>![<font style="font-size:15px"><i>Plot generated with parameters: output = "jpg", multi = FALSE, and verbose = TRUE</i></font>](img/output_jpg.jpg)</center>


The following code creates a level 5 plot for $aeid$ 80 and $spid$ 01504209 of output type "console". 
```{r mc_plot_console, eval = FALSE}
# Plot of Level 5 MC data for single aeid and spid and output to console.
tcplPlot(lvl = 5, # data level
         fld = c('aeid','spid'), # field to query on
         val = list(80, '01504209'), # value for each field
         # values should match their corresponding 'fld'
         multi = FALSE, # single plot per page, TRUE not valid with output = console
         verbose = FALSE, # do not output all details, TRUE not valid with output = console
         output = "console") # output in R console

```
<center>![<font style="font-size:15px"><i>Plot generated with parameters: output = "console", multi = FALSE, and verbose = FALSE</i></font>](img/output_console.png)</center>
