...
To support different data retrieval needs within <font face="CMTT10">tcpl</font>, there are a number of functions which query the database and return information to the local R session. Throughout this vignette, abbreviations will used to describe data retrieved from the database or to refer to processing steps. 'MC' is used for assay data with multiple concentrations whereas 'SC' describes single concentration. A particular data or processing level is indicated by appending the level id/number to the end of the 'SC' or 'MC' designation. For example, if discussing single concentration data from level 2 processing: 'SC2.' See tcpl's other vignettes for more information. 
