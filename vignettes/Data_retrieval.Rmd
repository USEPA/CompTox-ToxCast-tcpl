...
### Consider *in vivo* toxicity benchmarks as POD-Traditional
...
Next identify the observed lowest effect (based on statistically significant i.e. treatment related=1) and lowest observed adverse (i.e. critical) effect levels at minimum dose_adjusted (mg/kg/day) value
...
First, query the invitrodb database for all assay annotations, and filter results to consider only "liver" tissue-based endpoints.

```{r txct-annotations-liver-subset, warning=FALSE, eval=FALSE}
toxcast_annotations_subset <- tcplQuery("SELECT * FROM assay
                        INNER JOIN assay_component ON assay.aid = assay_component.aid
                        INNER JOIN assay_component_endpoint ON assay_component.acid=assay_component_endpoint.acid 
                        where assay.tissue='liver';")
```

For this subset of endpoints of targeted interest, pull assay results (mc5-mc6) for the chemical "Pentachlorophenol"
```{r txct-data-pull, warning=FALSE, message=FALSE, eval=FALSE}
# Load the chemical data from the database
...
Summarize ToxCast AC50 values as 5%, 50%, and mean.
...
Use the High-Throughput Toxicokinetics httk R package (https://cran.r-project.org/web/packages/httk/index.html) to generate administered equivalent doses (AEDs) for Toxcast summary AC50 values. Some considerations: 
...
- Restrictive clearance indicates that chemical bound to protein is relatively unavailable for hepatic metabolism or renal excretion (whereas non-restrictive clearance assumes that chemical bound to protein rapidly disassociates from that protein for metabolism and excretion). 
...
POD-Traditional (LEL and LOAEL) and POD-NAM (ToxCast-derived AEDs for 5%, 50%, and mean AC50 values) can be compared having been converted to mg/kg/day units. 
...
For this "Pentachlorophenol liver toxicity" example, POD-Traditional is more protective than the most sensitive POD-NAM.