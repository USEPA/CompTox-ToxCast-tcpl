...
<font face="CMTT10">tcplPlot</font> is tcpl’s single flexible plotting function, allowing for interactive yet consistent visualization of concentration-response curves via several customizable parameters. As a standalone plotting utility built with the R library <font face="CMTT10">plotly</font> to display the additional curve-fitting models, <font face="CMTT10">tcplPlot</font> implements the R library plumber to provide representational state transfer-application programming interface (“REST API”) functionality. The <font face="CMTT10">tcplPlot</font> function requires a level (`lvl`), field (`fld`), and value (`val`) input combination to load the necessary data and display the associated plots and tables as output. Level 4 plots the concentration-response series fit by all models, whereas Level 5 also includes the winning model highlighted with an activity hit call presented. Level 6 multi-concentration and single-concentration plotting are not currently supported by <font face="CMTT10">tcplPlot</font>. 

Customization of outputs is possible by specifying parameters, including `output`, `multi`,
`fileprefix`, `by`, `verbose`, `nrow`, `ncol`, and `dpi`.  
...
- The multi parameter allows for single or multiple plots per page. `Multi=TRUE` is the default option for PDF outputs, whereas Multi=FALSE is only option for other outputs. If `‘multi’ = TRUE`: default 4 plots per page for 'verbose' = TRUE and 6 plots per page for 'verbose' = FALSE.

- The `by` parameter indicates how files should be divided, typically by $aeid$ or $spid$. The `fileprefix` parameter allows the user to set a prefix of file when saving. Standard filenames are tcplPlot_sysDate().Output (tcplPlot_20230802.pdf), or can be customized as fileprefix _by_. Output (Plots_AEID.pdf) 
...
```
<center>![<font style="font-size:15px"><i>Plots with parameters: output = "pdf", multi = TRUE, and verbose = TRUE for aeid 80 and spid 01504209</i></font>](img/output_pdf.png)</center>
...
# Plot a verbose plot of Level 5 MC data for single aeid 80 and spid 01504209 and output as jpg.
tcplPlot(lvl = 5, # data level
         fld = c('aeid','spid'), # field to query on
...
<center>![<font style="font-size:15px"><i>Plot generated with parameters: output = "jpg" and verbose = TRUE for </i></font>](img/output_jpg.jpg)</center>

## Output to Console, by M4ID or AEID and SPID

Due to the dynamic nature of "m#"ids, the first example code chunk does not include a corresponding plot. Here, the $m4id$ value (482273) corresponds with the <font face="CMTT10">mc_vignette,/font> R data object. To run test this code, a valid $m4id$ value must be supplied. 

 
The second example includes a level 5 plot for endpoint and sample of output type “console”. Only 1 concentration series can be output in console at a time. 
...
# Plot of Level 5 MC data for single aeid (80) and spid (01504209) and output to console.
tcplPlot(lvl = 5, # data level
         fld = c('aeid','spid'), # field to query on
         val = list(80, '01504209'), # value for each field
         # values should match their corresponding 'fld'
