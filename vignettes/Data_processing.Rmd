...
# Primary Packages #
devtools::load_all() #use when dev versions are installed locally
#library(tcpl)
#library(tcplfit2)
...
For demonstrating the assignment of level 4 methods, method '1' will be specified to calculate that the BMAD and estimate one standard deviation of baseline using the two lowest concentration groups of **treatment wells**.
...
While the ToxCast pipeline supports bidirectional fitting, sometimes it is necessary to censor the hitcalls of curves fit in the biologically irrelevant direction. Level 5 now contains two methods for overwriting the hitcall value. If applied, these will overwrite the hitcall value for any biologically irrelevant curve by flipping it negative. 
