...
The <font face="CMTT10"> tcplRun </font> function will attempt to use multiple processors on Unix-based systems (does not include Windows). Depending on the system environment, or if the user is running into memory constraints, the user may wish to use less processing power. The user can specify less processing power by setting the "mc.cores" parameter in the <font face="CMTT10"> tcplRun </font> function.
...
## Methods
To promote reproducibility, all method assignments must occur through the database. Methods cannot be passed directly to either the <font face="CMTT10"> tcplRun </font> function or the low-level processing functions called by <font face="CMTT10"> tcplRun </font>. 

In general, methods data are stored in the "_methods" and "_id" tables that correspond to the data-storing tables. For example, the "sc1" table is accompanied by the "sc1_methods" table which stores the available methods for SC1, and the "sc1_aeid" table which stores the method assignments and execution order.
...
<font face="CMTT10"> tcpl_v2 </font> package only modeled responses in the positive direction. Therefore, a signal in the negative direction needed to be transformed to the positive direction during normalization. Creating multiple endpoints for one component was one way to enable multiple normalization approaches. Multiple normalization approaches were necessary when the assay component detected a signal in both positive and negative directions. Negative direction data was inverted by multiplying the final response values by ${-1}$ (see the "resp.mult.neg" methods in Table 2). <font face="CMTT10"> tcplFit2 </font> utilized within <font face="CMTT10"> tcpl_v3 </font> onward allows for bidirectional fitting, therefore the "resp.mult.neg" method is now only required in special use cases.
...
Before assigning the methods below, the user needs to register the data for the single-concentration assay, as shown in the Register and Upload New Data section.
...
Above, methods 1, 11, and 13 were assigned for both endpoints. The method assignments instruct the processing to: (1) calculate $\mathit{bval}$ for each assay plate ID by taking the median of all data where the well type equals "n;" (2) calculate a fold-change over $\mathit{bval}$; (3) log-transform the fold-change values with base 2.

If a user needs to add a method to the end of a normalization sequence, as shown above, then the user can use a second methods assignment statement.  For example, suppose for AEID 2 only we need to indicate a change of directionality in the response and multiply all response values by $-1$.  Then, the following code can be executed. **Reminder, the order of methods assignment matters, particularly in the normalization step.**
...
In the example above, we are assigning level 2 methods such that the cutoff value is $log_2(1.2)$. Thus, if the maximum median value ($\mathit{max\_med}$) is greater than or equal to the efficacy cutoff ($\mathit{coff}$), then the sample ID is considered active and the hit call ($\mathit{hitc}$) is set to 1.
...
htmlTable(output,
        ...
        tfoot="&dagger; Level 0 pre-processing occurs outside the package and specifics are covered in Introduction vignette. &ddagger;Level 6 processing *will not* be possible in  tcpl v3.0, but may be included in future versions")
...
Every assay component needs at least one transformation method assigned to complete level 2 processing.
...
```{r eval=FALSE}
mc3 <- tcpl::tcplPrepOtpt(tcplLoadData(lvl = 3,type = 'mc',
                                       fld = 'aeid',val = 80,add.fld = TRUE))
```

For demonstration purposes, we provide the `mc_vignette` R data object with in the `tcpl` package since we are not connected to such a database.  The `mc_vignette` object contains a subset of data from level 3 through 5 from the most recent version of the invitroDB database.  The following code demonstrates how to load the example data object and how to access/store the level 3 data in the `mc3` object.

```{r message=FALSE}
# load all the functions in the `tcpl` package
devtools::load_all()
# load the example data in the `tcpl` package
data(mc_vignette,package = 'tcpl')
# allocate the level 3 example data to `mc3`
mc3 <- mc_vignette[['mc3']]
```

Plot the concentration-response series for one of the example spid's in the `mc3` object and the summary estimates mentioned previously in the bullet list above.

```{r fig.align='center',class.source="scroll-100",message=FALSE}
## Obtain Data ##
...
A <- mc3 %>%
  ...
  geom_point(data = level3_summary,
             ...
             col = "paleturquoise3",alpha = 0.75,size = 2)+
...
E <- mc3 %>%
  ...
  ggplot(data = .,aes(logc,resp))+
  geom_point(pch = 1,size = 2)+
  geom_point(data = level3_min,aes(x = logc,y = resp),
             col = "blue",alpha = 0.75,size = 2)+
  geom_point(data = level3_max,aes(x = logc,y = resp),
             col = "red",alpha = 0.75,size = 2)+
  xlab(expression(paste(log[10],"(Concentration) ",mu,"M")))+
  ylab(mc3[,aenm])+
...
**Figure 1:** The plots in this figure illustrate the summary estimates calculated as part of the level 4 processing that occurs prior to dose-response modeling.  Each of the plots depict the observed concentration-response data with white circles, where the x-axis is a base 10 log-transformed concentration.  The mean response values for each concentration group are depicted as turquoise circles in the upper left plot.  Similarly, the upper right plot depicts the median response values for each concentration group with hot-pink circles. The middle two plots depict the maximum mean ($\mathit{max\_mean}$) and median ($\mathit{max\_med}$) response estimates (left and right plots, respectively).  The bottom left plot depicts the minimum response value ($\mathit{min\_resp}$) with a blue circle and the maximum response value ($\mathit{max\_resp}$) is with a red circle.  Finally, the minimum ($\mathit{min\_logc}$) and maximum $\mathit{max\_logc}$ log-scale concentrations are depicted, respectively, with a blue and red vertical dashed line. 
...
Each concentration-response series is fit with all ten parametric model available in <font face="CMTT10"> tcplFit2 </font>, see table below for further details:
...
**Figure 2:** This figure contains a simulated concentration-response curve to illustrate the general underlying curve shape(s) covered by each of the models included in the `tcplfit2` package and used on the back-end of the level 4 data processing in `tcpl`.  Each sub-plot in the figure corresponds to a single parametric model included in the model fitting process and has a corresponding color and line type to accompany it.  All sub-plots are plotted such that the x-axis represents the log-transformed concentration ($base=10$) and the y-axis represents the response values. 

#### Methods Assignment

Level 4 does not utilize any assay endpoint-specific methods; the user only needs to run the <font face="CMTT10"> tcplRun</font> function. **Level 4 processing and all subsequent processing is done by assay endpoint, not assay component**. The previous section showed how to find the assay endpoints for an assay component using the <font face="CMTT10"> tcplLoadAeid </font> function. We show two example of how a user can assign methods using <font face="CMTT10"> tcpl </font>.  First, we assign methods for a single assay endpoint.  Second, we assign methods for all assay endpoints within an assay.  Please note, the second approach can also be used for a subset of aeid's within an assay that is larger than one.

For both examples, the level 4 methods assigned include the bmad estimation (mthd_id = 1) and the required one standard deviation estimation (mthd_id = 3), mentioned in the pre-modeling process section, for the desired set of assay endpoints.  Here, method '1' specifies that the BMAD is estimated using the two lowest concentration groups of **treatment wells**.  Additionally, the assignment specifies that the BMAD estimation should be applied prior to estimating the baseline standard deviation.

```{r eval=FALSE}
## Assign the Level 4 Processing Methods Single AEID ##
tcpl::tcplMthdAssign(
    lvl = 4,
    id = 80, # aeid = 80 is the only assay endpoint we want to assign methods
...
We can now load the level 4 data from the database.
```{r eval=FALSE}
mc4 <- tcpl::tcplPrepOtpt(tcplLoadData(lvl = 3,type = 'mc',
                                       fld = 'aeid',val = 80,add.fld = TRUE))
```
...
```{r eval=FALSE}
# Load Processed Level 4 Data
mc4 <- tcpl::tcplPrepOtpt(
  tcpl::tcplLoadData(
    lvl = 4L,
    type = 'mc',
    fld = 'aeid',
...
estDR <- cbind.data.frame(X,estDR) %>%
  reshape2::melt(data = .,measure.vars = c(
    "poly1","poly2","power","hill","gnls","exp2","exp3","exp4","exp5"
  ))
...
tmp <- mc4 %>% 
  ...
  apply(.,MARGIN = 1,FUN = function(x){
    cell_spec(x,color = ifelse(x == min(x),yes = "blue",no = "black"))
  }) %>% t() %>% 
  data.frame() %>% 
  cbind.data.frame(mc4[,dsstox_substance_id],.)
colnames(tmp) <- colnames(mc4)[grep(colnames(mc4),pattern = "aic")] %>%
  str_remove(.,pattern = "_aic") %>% c("dsstox_id",.)

tmp %>%
  kbl(escape = F,format = 'html',centering = TRUE,
      caption = "Table 9: AIC values from All Fits from Level 4.") %>%
...
Here, we are going to assign the Attagene assay endpoints all with the same methods.  However, one can assign methods for a single assay endpoint or a subset of all endpoints within an assay.
...
The above chunk of code assigns <provide description of assigned methods...>.

Since the continuous hit call (as described previously) is a probability the value is between 0 and 1.  The higher the probability indicates the more probable that the concentration-response series indicates biological activity in the measured response (i.e. 'active' hit).

<Information about the point of departure (POD).(??)>
...
Code to load level 5 multiple concentration data from the database for a single assay endpoint (aeid).

```{r eval=FALSE}
mc5 <- tcpl::tcplPrepOtpt(tcplLoadData(lvl = 5,type = 'mc',
                                       fld = 'aeid',val = 80,add.fld = TRUE))
```

We have provided a subset of the 'mc5' data within the 'mc_vignette' R data object within the 'tcpl' package.

```{r}
mc5 <- mc_vignette[["mc5"]]
```

Visual display of the model fitting results with the tcpl package when connected to the/a database.

```{r eval=FALSE}
my_id <- tcplLoadData(lvl = 5,fld = c('aeid','spid'),val = list(80,'01504209'))$m4id
tcplPlot(lvl = 5,val = my_id,output = 'console')
```

Visualize model fitting results and potency estimates from Level 4 multiple-concentration (mc) data.
...
In this section we provide a practical step-by-step example for single- and multi-concentration data from methods assignment through data processing.

### Single-concentration Data

After level 0 is loaded into the database, we need to assign the data processing methods for each step.  In the following code chunk, we demonstrate how to assign the methods for each step.
...
After level 0 multiple-concentration data is loaded into the database, we need to assign the data processing methods for each step.  In the following code chunk, we demonstrate how to assign the methods for each step.
