...
they occur
* The number of medians responses greater than $3\mathit{bmad}$.

When referring to the concentration series , the "mean" and "median" response values are defined as the mean or median of all observed response values at each concentration. In other words, the maximum median is the maximum of all median response values across the concentration groups in the series. See the simulated concentration-response series in the figures below illustrating these summary values.

```{r,eval=FALSE}
tcpl::tcplRun(slvl = 4,elvl = 5,type = 'mc')
```
...
set.seed(854)
a <- round(rnorm(n = 1,mean = 0.75),3)
p <- 3
# Set-up Dummy Response
conc <- c(.03, .1, .3, 1, 3, 10, 30, 100)
X <- rep(conc, each = 3)
set.seed(658)
Y <- a*log10(X)^p + abs(a*log10(conc[1])^p) + rt(n = length(X),df = 4)
# Plot the Dummy Example
plot(X,Y,log = "x",
     xlab = "log10(Concentration)",ylab = "Response",
     main = "Dummy Observed Concentration-Response",
     ylim = c(-2,16))
```

```{r}
# Obtain the "Baseline" SD
onesd <- sd(Y[which(X %in% conc[1:2])])
# plot the onesd with the dummy example
plot(X,Y,log = "x",
     xlab = "log10(Concentration)",ylab = "Response",
     main = "Dummy Observed Concentration-Response",
     ylim = c(-2,16))
abline(h = c(onesd*(-1),onesd),lty = "dashed",col = rep('lightgray',2))
abline(h = c(onesd*(-1),onesd)*1.349,lty = "dashed",col = rep('indianred',2))
legend("topleft",lty = rep('dashed',2),col = c('lightgray','indianred'),
        legend = c('1 SD (lowest 2 conc)','bmr = 1.349*(1 SD)'))
```

```{r}
# Set-up the data for how tcplfit2 needs the data
row <- list(conc = X,
             resp = Y,
             bmed = 0,
             cutoff = 1,
             ...
             assay = "dummy assay")
# Fit the Data
fit1 <- tcplfit2::concRespCore(row = row)
# Print the fit output
fit1
```

```{r}
# Plot Dummy Data with Annotations (x-axis log-scale)
plot(X,Y,log = "x",
     xlab = "log10(Concentration)",ylab = "Response",
     ylim = c(-2,16))
abline(h = c(onesd*(-1),onesd),lty = "dashed",col = rep('lightgray',2))
abline(h = c(-1,1)*fit1[,"bmr"],lty = "dashed",col = rep('indianred',2))
points(conc,fit1[,"a"]*conc^fit1[,"p"],col = 'blue')
lines(conc,fit1[,"a"]*conc^fit1[,"p"],col = 'blue')
segments(x0 = fit1[,"bmd"],x1 = fit1[,"bmd"],y0 = -5,y1 = fit1[,"bmr"],
         col = 'red',lwd= 1.5)
# true underlying fit
# points(conc,a*log10(conc)^p + abs(a*log10(conc[1])^p),col = 'green')
# lines(conc,a*log10(conc)^p + abs(a*log10(conc[1])^p),col = 'green')
```
