...
Where $y_i$ is the $i^{th}$ observation of all treatment wells in the assay component, i.e. well type ($\mathit{wllt}$) equal to "t" - $wllt = t$, and $\tilde{y}$ is the median across all $y_i$'s.  The constant value, $1.4826$, is the default adjustment value used in the underlying R function to ensure $bmad$ is a consistent estimator of the standard deviation ($\sigma$) assuming the sample size ($N$) of the observations is large and they are normally distributed (i.e. Gaussian), see \{citation?\}.

<NOT SURE HOW THIS IS APPLICABLE  -- NEED TO CHECK>
...
Where $y_{i}$ is the $i^{th}$ baseline observation, defined based on the assigned method - see Table 5 for details, and $\tilde{y}$ is the median of all the baseline observations.  The constant value, $1.4826$, is the default adjustment value used in the underlying R function to ensure $bmad$ is a consistent estimator of the standard deviation ($\sigma$) assuming the sample size ($N$) of the baseline observations is large and they are normally distributed (i.e. Gaussian), see \{citation?\}.
...
```{r warning = FALSE, echo = FALSE}
Level4_MethodAssignment <- c(
  1,2,3
)

GeneralDescription <- c(
...
output <- 
  data.frame(Level4_MethodAssignment,GeneralDescription,BaselineObservations,ID,Details)
...
htmlTable(output,
        ...
        css.cell =  ' padding-bottom: 5px;  vertical-align:top; padding-right: 10px;min-width: 5em ',
          ...
          caption="Table 5: BMAD estimation details by method assignment."
)
...
htmlTable(output,
        ...
        css.cell =  ' padding-bottom: 5px;  vertical-align:top; padding-right: 10px;min-width: 5em ',
          ...
          caption="Table 6: Model output descriptions." ,
...
<Introduction description for the example model fits.>
...
mc3 %>% 
  ...
  geom_line(data = estDR,
            aes(x = log10(X),y = value,colour = variable),
            alpha = 0.5)+
  labs(colour = "Models")+
  scale_colour_manual(values = fit_cols)+
  xlab("Log Concentration")+
...
tmp %>%
  kbl(escape = F,format = 'html',centering = TRUE,
      caption = "Table of the AIC values from All Fits from Level 4.") %>%
...
basePlot <- mc3_ss %>% 
  ...
  ggplot()+
  geom_point(aes(x = logc,y = resp),pch = 1)+
  # Cutoff Band
...
```{r eval=FALSE}
sc_res <- tcplRun(id = 1, slvl = 1, elvl = 2, type = "sc")
```
...
mc_res_0.3 <- tcpl::tcplRun(
  id = bsk.acid[,acid], # assay component id to pipeline
  type = "mc",
