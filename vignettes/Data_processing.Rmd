...
output <- 
  data.frame(Level4_MethodAssignment,BaselineObservations,ID,Details)
...
```{r eval=FALSE}
tmp1 <- mc3 %>% 
  dplyr::filter(.,aeid == 98) %>% 
  dplyr::filter(.,spid == "01504064")

tmp1mean <- tmp1 %>% group_by(.,cndx) %>% 
  summarise_at(.,.vars = c('logc','resp'),.funs = mean)
tmp1median <- tmp1 %>% group_by(.,cndx) %>% 
  summarise_at(.,.vars = c('logc','resp'),.funs = median)

tmp2 <- mc4 %>% 
  dplyr::filter(.,aeid == 98) %>% 
  dplyr::filter(.,spid == "01504064")

ggplot2::ggplot(tmp1,aes(logc,resp))+
  geom_point()+
  geom_point(data = tmp1mean,col = "red",alpha = 0.25)+
  geom_point(data = tmp1median,col = "blue",alpha = 0.25)+
...
```{r,echo=FALSE}
# Quadratic Dummy Model
a <- 6.5
b <- 2.5
# Set-up the Dummy Conc Response
conc <- c(.03, .1, .3, 1, 3, 10, 30, 100)
X <- rep(conc, each = 3)
set.seed(658)
Y <- a*(log10(X)/b + (log10(X)/b)^2) + rt(n = length(X),df = 4)
# Obtain the Mean and Median Responses
Y.mean <- unlist(lapply(X = conc,function(x){mean(Y[which(X == x)])}))
Y.median <- unlist(lapply(X = conc,function(x){median(Y[which(X == x)])}))
```

<Copy an example concentration response series from the DB mc level 3, 4, 5. - Refer back to the markdown from BSK examples.>

```{r,echo=FALSE}
par(mfrow = c(1,2))
# Plot the Data
plot(X,Y,log = "x",
     xlab = "log10(Concentration)",ylab = "Response")
points(conc,Y.mean,col = "red")
points(conc,Y.median,col = "blue")
legend("topleft",bty = 'n',
       legend = c("mean response","median response"),
       col = c("red","blue"),pch = 1)

plot(X,Y,log = "x",
     xlab = "log10(Concentration)",ylab = "Response")
points(conc[which(Y.mean == min(Y.mean) | Y.mean == max(Y.mean))],
       c(min(Y.mean),max(Y.mean)),col = 'red',pch = c(25,24))
points(conc[which(Y.median == min(Y.median) | Y.median == max(Y.median))],
       c(min(Y.median),max(Y.median)),col = 'blue',pch = c(25,24))
legend("topleft",bty = 'n',
       legend = c("min mean response","max mean response",
                  "min median response","max median response"),
...
Assign the level 4 methods to be used for bmad estimation (mthd_id = 1) and one standard deviation (mthd_id = 3) for the desired set of assay endpoints.  Here, we method '1' specifies that the BMAD is estimated using the two lowest concentration groups of treatment wells.  Additionally, the assignment specifies that the BMAD estimation should be applied prior to estimating the baseline standard deviation.

```{r eval=FALSE}
## Assign the level 4 processing Methods ##
tcpl::tcplMthdAssign(
    lvl = 4,
    id = tmp.ids,
...
With the methods assigned, the level 4 processing can be complted for the desired set of assay endpoints (aeid).
