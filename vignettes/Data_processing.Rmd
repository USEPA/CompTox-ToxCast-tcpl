...
Where $y_{i}$ is the $i^{th}$ baseline observation, defined based on the assigned method - see the following table, and $\tilde{y}$ is the median of all the baseline observations.
...
Code to load level 3 multiple concentration data from the database for a single assay endpoint (aeid).
...
We have provided a subset of the 'mc3' data within the 'mc_vignette' R data object within the 'tcpl' package.
...
Plot the concentration-response series for one of the spid's in the `mc3` object.

```{r eval=FALSE,class.source = "scroll-100"}
tmp <- mc3 %>%
  dplyr::select(.,c(spid,chnm,logc,resp)) %>%
  dplyr::group_by(spid,chnm,logc) %>% 
  dplyr::summarise(mean_resp = mean(resp),med_resp = median(resp)) %>% 
  reshape2::melt(.,id.vars = c('spid','chnm','logc'))

tmp_min <- mc3 %>%
  dplyr::group_by(spid,chnm) %>% 
  dplyr::filter(resp == min(resp)) %>%
  dplyr::mutate(variable = "resp_min",value = resp) %>% 
  dplyr::select(.,c(spid,chnm,logc,variable,value))
tmp_max <- mc3 %>% 
  dplyr::group_by(spid,chnm) %>% 
  dplyr::filter(resp == max(resp)) %>% 
  dplyr::mutate(variable = "resp_max",value = resp) %>% 
  dplyr::select(.,c(spid,chnm,logc,variable,value))

tmp <- rbind.data.frame(tmp,tmp_min,tmp_max)
```
...
tmp %>% 
  ...
  ggplot()+
  geom_point(data = dplyr::filter(mc3,spid == "01504209"),aes(logc,resp),alpha = 0.25)+
  geom_point(aes(logc,value,col = variable))+
...
A <- mc3 %>%
  ...
  geom_point(data = tmp,aes(x = logc,y = mean_resp),col = "red",alpha = 0.5)+
...
B <- mc3 %>%
  ...
  ggplot(data = .,aes(logc,resp))+
  geom_point(alpha = 0.25)+
  geom_point(data = tmp,aes(x = logc,y = med_resp),col = "blue",alpha = 0.5)+
  xlab("Log Concentration")+
  ylab(mc3[,aenm])+
...
C <- mc3 %>%
  ...
  ggplot(data = .,aes(logc,resp))+
  geom_point(alpha = 0.25)+
  geom_point(data = tmp_min,aes(x = logc,y = resp),
             pch = 2,col = "blue")+
  geom_point(data = tmp_max,aes(x = logc,y = resp),
             pch = 6,col = "red")+
  xlab("Log Concentration")+
  ylab(mc3[,aenm])+
...
D <- mc3 %>%
  ...
  ggtitle(label = "Minimum & Maximum\nConcentrations")+
  theme_bw()
# 'max_mean', 'max_mean_conc', 'max_med', & 'max_med_conc'
E <- mc3 %>%
  dplyr::filter(spid == "01504209") %>% 
  ggplot(data = .,aes(logc,resp))+
  geom_point(alpha = 0.25)+
...
<See the concentration-response series in the figures below illustrating these summary values.>

<Plot demonstrating each of the summarized values or at least the major ones.>
...
Visuzalize model fitting results by creating the figures manually from data.
```{r fig.align='center',class.source = "scroll-100"}
mc3_ss <- mc3 %>% dplyr::filter(spid == "01504209")
mc4_ss <- mc4 %>% dplyr::filter(spid == "01504209")
mc5_ss <- mc5 %>% dplyr::filter(spid == "01504209")
...
bestfit_col <- 'royalblue'
names(bestfit_col) <- mc5_ss[,modl]

basePlot <- mc3_ss %>% 
  ggplot()+
  geom_point(aes(x = logc,y = resp))+
  # Cutoff Band
  ...
  # Other Model Fits
  geom_line(data = dplyr::filter(estDR,variable != mc5_ss[,modl]),
            aes(x = log10(X),y = value,lty = variable),alpha = 0.15,show.legend = TRUE)+
  # Legend Information
  labs(lty = "Model",color = "Best Fit")+
  # Titles and Labels
  xlab("Log Concentration")+
  ylab(mc4[which(mc4[,spid] == "01504209"),aenm])+
...
```{r}
potency_cols <- colorspace::qualitative_hcl(n = 5) %>%
  colorspace::simulate_cvd(.,colorspace::deutanomaly_cvd[[2]])
...
basePlot+
  ...
  geom_hline(data = mc5_ss,
             ...
             alpha = 0.5,col = potency_cols[3])+
  geom_segment(data = mc5_ss,
               aes(x = log10(ac50),xend = log10(ac50),y = -0.5,yend = max_med*0.5),
               alpha = 0.5,col = potency_cols[3])+
  geom_point(data = mc5_ss,
  ...
  geom_hline(data = mc5_ss,
             ...
             alpha = 0.5,col = potency_cols[4])+
  geom_segment(data = mc5_ss,
               aes(x = log10(ac10),xend = log10(ac10),y = -0.5,yend = max_med*0.1),
               alpha = 0.5,col = potency_cols[4])+
  geom_point(data = mc5_ss,
  ...
  geom_hline(data = mc5_ss,
             ...
             alpha = 0.5,col = potency_cols[5])+
  geom_segment(data = mc5_ss,
               aes(x = log10(ac5),xend = log10(ac5),y = -0.5,yend = max_med*0.05),
               alpha = 0.5,col = potency_cols[5])+
  geom_point(data = mc5_ss,
...
```{r eval=FALSE}
potency_dose <- mc5_ss %>% 
  mutate(log10_bmd = log10(bmd),
         log10_acc = log10(acc),
         log10_ac50 = log10(ac50),
         log10_ac10 = log10(ac10),
         log10_ac5 = log10(ac5)) %>% 
  select(c(dsstox_substance_id,log10_bmd,log))
