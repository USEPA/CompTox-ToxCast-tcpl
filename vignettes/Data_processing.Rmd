...
vignette: >
  %\VignetteIndexEntry{3. Data Processing}
...
```{r eval = TRUE, hide=TRUE, message = FALSE, echo=FALSE, warning=FALSE}
# Primary Packages #
...
This vignette is intended to help the user understand the general aspects of how the data are processed before diving into the specifics of each processing level for both screening paradigms (i.e. processing types). The details of the two screening paradigms are provided in later sections.
...
Sometimes it is necessary to prevent active hit calls from $\mathit{max\_med}$ > cutoff in the biologically irrelevant direction. Level 2 now contains two methods for overwriting the $\mathit{max\_med}$ value. If applied, these will prevent an active hitcall value for any $\mathit{max\_med}$ > cutoff in the biologically unintended direction. Here, they replace $\mathit{max\_med}$ with either a true minimum or maximum rather than the greatest absolute value median. 
...
Method_Name <- c(
  "ow_bidirectional_gain", "ow_bidirectional_loss"
)
Description <- c(
...
htmlTable(output,
          ...
          rnames = FALSE  ,
          css.cell =  ' padding-bottom: 5px;  vertical-align:top; padding-right: 10px;min-width: 5em ',
          caption="Table 5: Level 2 overwrite (ow) methods for preventing an active hitcall when max_med > cutoff in the unintended direction."
          )
```
...
htmlTable(output,
        ...
        align = 'l',
        align.header = 'l',
        caption="Table 6: Summary of the tcpl multiple-concentration pipeline.",
...
htmlTable(output,
          ...
          rnames = FALSE  ,
          css.cell =  ' padding-bottom: 5px;  vertical-align:top; padding-right: 10px;min-width: 5em ',
          caption="Table 7: Multiple concentration *baseline* variability estimation by method assignment in Level 4 processing."
          )
```
...
When referring to the concentration series , the "mean" and "median" response values are defined as the mean or median of all observed response values at each concentration. In other words, the maximum median is the maximum of all median response values across the concentration groups in the series. Similarly, the minimum median is the true minimum (or max in the negative direction) of all median response values across the concentration groups in the series.
...
After the summary values are obtained for each concentration-response series, all ten parametric models available in <font face="CMTT10"> tcplFit2 </font> are used to fit each series.  See Table 8 for model details.
...
htmlTable(output,
        ...
        css.cell =  ' padding-bottom: 5px;  vertical-align:top; padding-right: 10px;min-width: 5em ',
          ...
          caption="Table 9: Model output descriptions." ,
...
For demonstrating the assignment of level 4 methods, method '1' will be specified to calculate that the BMAD and estimate one standard deviation of baseline using the two lowest concentration groups of **treatment wells** (well type, or wllt, equal to "t").
...
tcpl::tcplMthdAssign(
    lvl = 4,
    id = atg.aeid[,aeid],
    mthd_id = c(1), # the set of methods to be assigned
    ordr = 1, # order in which they should be assigned
    type = "mc"
  )
```
...
```{r fig.align='center',fig.dim=c(8,5.5),class.source = "scroll-100", warnings=FALSE}
## Create a Sequence of Concentration Values within Observed Range ##
X <- seq(
 mc4[which(mc4[,spid] == "01504209"),conc_min],
 mc4[which(mc4[,spid] == "01504209"),conc_max],
 length.out = 100
)
## Obtain the Continuous Fit of Level 4 Model Estimates ##
...
Level 5 processing determines the winning model and activity for the concentration series, bins all of the concentration series into categories, and calculates potency estimates based on the activity cutoff.

### Methods Assignment

**The model with the lowest AIC value is selected as the winning model** ($\mathit{modl}$ ), and is used to determine the activity (or hit call) for the concentration series. If two models have equal AIC values, then the simpler model (i.e. model with fewer parameters) wins. Additionally, if the constant model describes the response best out of all the models fit then $\mathit{modl}$ is reported as 'none'. Table 10 provides a side-by-side comparison of AIC values from each of the ten model fits for the concentration-response series we include in the example dataset <font face="CMTT10"> mc_vignette</font>.
...
mc4_aic %>%
  kbl(
    ...
    format = 'html',
    centering = TRUE,
    caption = "Table 10: AIC values from All Fits in Level 4.  Values highlighted in blue indicate the winning model for the compound.") %>%
  kable_styling(
    font_size = 14,
    c("striped","hover"),
    full_width = FALSE
...
The estimated parameters from the winning model are stored in their respective parameter fields at level 5.  The activity of each concentration-response series is determined by calculating a continuous hit call that may be further binarized into active or inactive, depending on the level of stringency required by the user; herein, hitc < 0.9 are considered inactive. This cutoff was determined from extensive analysis of high-throughput transcriptomics (HTTr) data analyzed using tcplfit2, and comparison to known positive and negative reference chemicals. Continuous hitc in tcplfit2 [(Sheffield et al.)], 2021](https://doi.org/10.1093/bioinformatics/btab779) are defined as the product of three proportional weights: 1) the AIC of the winning model is better than the constant model (i.e. winning model is not fit to background noise), 2) at least one concentration has a median response that exceeds cutoff, and 3) the top from the winning model exceeds the cutoff. The constant model may never be selected as the winning model, but if the constant model has the lowest AIC compared to other models, the calculated continuous hitc will be zero. Though the hitc tend to approach 0 or 1, and are not normally distributed as a set of values, users may interpret the continuous hitc into active or inactive designations based on different thresholds. Further testing through implementation of this new functionality may reveal appropriate thresholds for different applications or assays. Hitc should not be interpreted as a probability (i.e., a hitc of 0.7 does not indicate a 70% chance of an active hitc).
...
While the ToxCast pipeline supports bidirectional fitting, sometimes it is necessary to censor the hitc of curves fit in the biologically irrelevant direction. Level 5 now contains two methods for overwriting the hitc value. If applied, these will overwrite the hitc value for any biologically irrelevant curve by flipping the hitc to a negative value. 
...
As described previously, since the continuous hitc is the product of three proportional weights, and the resulting value is between 0 and 1.  Hitc that approach 1 indicate concentration-response series more likely associated with true biological activity in the measured response rather than noise (i.e., an 'active' hit).

For each concentration series, several potency estimates are calculated on the winning model. The primary estimates include: (1) the activity concentration at the specified benchmark response (BMR) ($\mathit{bmd}$), (2) the activity concentration at $50\%$ of the maximal response ($\mathit{ac50}$), (3) the activity concentration at the efficacy cutoff ($\mathit{acc}$), (4) the activity concentration at $10\%$ of the maximal response, and (5) the concentration at $5\%$ of the maximal response. There are several additional potency and uncertainty estimates included as part of the level 5. The potency estimates are summarized in Figure 4. The winning model may return a $\mathit{bmd}$ estimate that falls outside of the tested concentration range, so bounds are placed to censor the estimate values. The lower and upper bounds for $\mathit{bmd}$ estimates are $0.1*\text{the lowest test concentration}$ and $10*\text{the the highest test concentration}$, respectively. If the calculated $\mathit{bmd}$ estimate is below or above the lower or the upper bounds, the value at the bound will be returned as the bounded $\mathit{bmd}$ estimate instead.
...
The level 5 data includes fields for the best model fit, the potency estimates, other estimates from the best model fit, as well as the ID fields.  A complete list of level 5 fields is available in the Introduction vignette.
