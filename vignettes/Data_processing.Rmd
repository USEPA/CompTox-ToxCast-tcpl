...
Level 4 processing splits the data into concentration-response series by sample and assay endpoint, then models the activity of each series. Each series is modeled using the bi-directional fitting methods available in the <font face="CMTT10"> tcplFit2 </font> R package \{citation?\}. Bi-directional fitting allows assay endpoints to have their activity patterns modeled directly and do not require any response curve inversion (i.e. observed responses producing a 'negative/decreasing' trend with increasing concentration groups are multipled by $-1$ to induce a 'positive/increasing' trend).

#### Processing

Level 4 processing starts with the removal of well types with only one concentration (e.g. empty wells - 'wllt = b'). Next a noise-band is establish for the assay endpoint using the baseline median absolute deviation ($\mathit{bmad}$).  The $\mathit{bmad}$ is calculated as the median absolute deviation (MAD) of the response values in the lowest two concentrations (i.e. concentration index is 1 or 2) for test compounds. The calculation to define $\mathit{bmad}$ is done once across the entire assay endpoint. **If additional data is added to the database for an assay component, the $\mathit{bmad}$ values for all associated assay endpoints will change.** Note, this $\mathit{bmad}$ definition is different
from the $\mathit{bmad}$ definition used for single-concentration screening.

$$ bmad_{mc} = median( \big| y_i - \hat{y} \big |)$$


\< Need to add in an equation for the Multi-Concentration BMAD calculation. \>
...
Equations <- c(
  ...
  "$f(x) = ax^p$", # power
  "$f(x) = tp/[1 + (\\frac{ga}{x})^{p}]$", # hill
  "$f(x) = \\frac{tp}{(1 + (ga/x)^{p} )(1 + (x/la)^{q} )}$", # gain-loss
  "$f(x) = ps[1]*(exp(\\frac{x}{ps[2]}) - 1)$", # exp 2
  "$f(x) = ps[1]*(exp((\\frac{x}{ps[2]})^{ps[3]}) - 1)$", # exp 3
  "$f(x) = ps[1]*(1-2^{\\frac{-x}{ps[2]}})$", # exp 4
  "$f(x) = ps[1]*(1-2^{(-(\\frac{x}{ps[2]})^{ps[3]})})$" # exp 5
)

Description <- c(
  "Constant: Fits a constant line and returns generic model outputs. success = 1 for a successful fit, 0 if optimization failed, and NA if nofit = TRUE. aic, rme, and er are set to NA in case of nofit or failure. pars always equals 'er'.", # constant
  "Polynomial-linear: Fits to f(x) = a*x and returns generic model outputs. Zero background and increasing absolute response are assumed. Parameters are 'a' (y scale) and error term 'er'. success = 1 for a successful fit, 0 if optimization failed, and NA if nofit = TRUE. cov = 1 for a successful hessian inversion, 0 if it fails, and NA if nofit = TRUE. aic, rme, modl, parameters, and parameter sds are set to NA in case of nofit or failure.", # linear 
  "Polynomial-quadratic: Fits to f(x) = a*(x/b + x^2/b^2) and returns generic model outputs. Zero background and monotonically increasing absolute response are assumed. Parameters are 'a' (y scale), 'b' (x scale), and error term 'er'. success = 1 for a successful fit, 0 if optimization failed, and NA if nofit = TRUE. cov = 1 for a successful hessian inversion, 0 if it fails, and NA if nofit = TRUE. aic, rme, modl, parameters, and parameter sds are set to NA in case of nofit or failure.", # quadratic
  "Power: Fits to f(x) = a*x^p and returns generic model outputs. Zero background and monotonically increasing absolute response are assumed. Parameters are 'a'(y scale), 'p' (power), and error term 'er'. success = 1 for a successful fit, 0 if optimization failed, and NA if nofit = TRUE. cov = 1 for a successful hessian inversion, 0 if it fails, and NA if nofit = TRUE. aic, rme, modl, parameters, and parameter sds are set to NA in case of nofit or failure.", # power
  "Hill: Fits to f(x) = tp/[(1 + (ga/x)^p )] and returns generic model outputs. Concentrations are converted internally to log10 units and optimized with f(x) = tp/(1 + 10^(p*(gax)) ), then ga and ga_sd are converted back to regular units before returning. Zero background and increasing initial absolute response are assumed. Parameters are 'tp' (top), 'ga' (gain AC50), 'p'(gain power), and error term 'er'. success = 1 for a successful fit, 0 if optimization failed, and NA if nofit = TRUE. cov = 1 for a successful hessian inversion, 0 if it fails, and NA if nofit = TRUE. aic, rme, modl, parameters, and parameter sds are set to NA in case of nofit or failure.", # hill
  "Gain-Loss: Fits to f(x) = tp/[(1 + (ga/x)^p )(1 + (x/la)^q )] and returns generic model outputs. Concentrations are converted internally to log10 units and optimized with f(x) = tp/[(1 + 10^(p*(gax)) )(1 + 10^(q*(x-la)) )], then ga, la, ga_sd, and la_sd are converted back to regular units before returning. Zero background and increasing initial absolute response are assumed. Parameters are 'tp' (top), 'ga' (gain AC50), 'p' (gain power), 'la' (loss AC50),'q' (loss power) and error term 'er'. success = 1 for a successful fit, 0 if optimization failed, and NA if nofit = TRUE. cov = 1 for a successful hessian inversion, 0 if it fails, and NA if nofit = TRUE. aic, rme, modl, parameters, and parameter sds are set to NA in case of nofit or failure." , # gain-loss
                 "Exponential-2: Fits to f(x) = ps[1]*(exp(x/ps[2]) - 1)", # exp2
                 "Exponential-3: Fits to f(x) = ps[1]*(exp((x/ps[2])^ps[3]) - 1)", # exp3
                 "Exponential-4: Fits to f(x) = ps[1]*(1-2^(-x/ps[2]))", # exp4
                 "Exponential-5: Fits to f(x) = ps[1]*(1-2^(-(x/ps[2])^ps[3]))") # exp5


output <- 
  data.frame(Model, Abbreviation, Equations, Description)
