...
```{r, message = FALSE, warning = FALSE, fold=TRUE}
# Primary Packages #
library(tcpl)
library(tcplfit2)
...
In addition to storing the data, the `invitrodb` stores every processing and analysis decision at the assay component or assay endpoint level to facilitate transparency and reproducibility. For illustrative purposes of this vignette, a small subset of actual data from the ToxCast program is featured throughout this vignette. `tcplLite` is no longer supported by `tcpl` because [`tcplfit2`](https://cran.r-project.org/package=tcplfit2) can be used to curve-fit data and make hit calls independent of `invitrodb`. `tcplLite` relied on flat files structured like `invitrodb` to produce curve-fitting and summary information like hit calls and AC~50~ values. Functionally [`tcplfit2`](https://cran.r-project.org/package=tcplfit2) replaces `tcplLite` because interested stakeholders can now curve-fit data and reproduce curve-fitting results independent of the `invitrodb` schema. For the ToxCast program, it is still important to use `invitrodb` when curve-fitting as `invitrodb` serves as a data resource for tracking pipelining decisions and providing a dataset for many interested stakeholders. Using `tcpl`, the user can upload, process, and retrieve data by connecting to a MySQL database. Additionally, past versions of the ToxCast database, containing all the publicly available ToxCast data, are available for download at: <https://www.epa.gov/comptox-tools/exploring-toxcast-data>.
...
* $assay\_name$ ($anm$)
* $assay\_footprint$
* $assay\_component\_name$ ($aenm$)
* $assay\_component_endpoint\_name$ ($aenm$)
...
Once the Level 0 data are loaded, data processing occurs via the `tcplRun()` function for both SC and mc multiple-concentration screening. `tcplRun()` can either take a single ID ($acid$ or $aeid$, depending on the processing type and level) or an $asid$. If given an $asid$, the `tcplRun()` function will attempt to process all corresponding components/endpoints. When processing by $acid$ or $aeid$, the user must supply correct [ID for each level](#lvl_ids).
...
`tcplVarMat()` returns a list of chemical by assay matrices (data.tables) where the rows are given by the dsstox_substance_id and corresponding chemical name ($chnm$) columns and the column names are given by assay endpoint name ($aenm$). To export this list, save to a .xlsx. Each matrix will have its own sheet.
